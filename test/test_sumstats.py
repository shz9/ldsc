...
def t(attr): return lambda obj: getattr(obj, attr, float('nan'))
...
class Test_H2_Statistical(unittest.TestCase):
    ...
    def test_prop(self):
        for h in [self.h2, self.h2_noint]:
            assert np.all(np.nanmean(list(map(t('prop'), h)),
                                     axis=0) - [1/3, 2/3] < 0.02)

    def test_prop_se(self):
        for h in [self.h2, self.h2_noint]:
            assert np.all(np.nanmean(list(map(t('prop_se'), h)), axis=0) -
                          np.nanstd(list(map(t('prop'), h)), axis=0) < 0.02)
